...
//    pc_ptr_.reset();
  ...
  void Cluster(const float* category_pt_data,
    ...
    size_t tot_point_num = cloud_size;
    valid_indices_in_pc_ = valid_indices;
//    CHECK_LE(valid_indices_in_pc_->size(), tot_point_num);
    point2grid_.assign(cloud_size, -1);
    ...
    for (size_t i = 0; i < cloud_size; ++i) {
      ...
      int pos_x = F2I(point[1], range_, inv_res_x_);  // col
      int pos_y = F2I(point[0], range_, inv_res_y_);  // row
...
//        temp["category_pt"].append(Json::Value(category_pt_data[grid]));
    ...
    for (size_t i=0; i < obstacles_.size(); ++i) obstacles_[i].clear();
    obstacles_.clear();
    id_img_.assign(grids_, -1);
    for (int row = 0; row < rows_; ++row) {
      for (int col = 0; col < cols_; ++col) {
...
//      std::vector<int> obs = obstacles_[obstacle_id];
      double score = 0.0;
      double height = 0.0;
...
//      obs->cloud.reset(new apollo::perception::pcl_util::PointCloud);
  ...
  void Classify(const float* classify_pt_data) {
    ...
    for (size_t obs_id = 0; obs_id < obstacles_.size(); obs_id++) {
      ...
      int meta_type_id = 0;
      for (int k = 0; k < num_classes; k++) {
        meta_type_probs[k] /= obstacles_[obs_id][grid_id].size();
        if (meta_type_probs[k] > meta_type_probs[meta_type_id]) {
          meta_type_id = k;
        }
      }
