...
cdef extern from "cluster2d.cpp" namespace "apollo":
    cdef cppclass Cluster2D:
        ...
        int points_num
        const float* pc_ptr_
        const int* valid_indices_in_pc_
        vector[int] point2grid_
        vector[int] id_img_
        vector[vector[int]] obstacles_
        vector[int] grids
        vector[vector[float]] meta_type_probs
        vector[float] score_
        vector[float] height_
        vector[string] type_name
        vector[vector[float]] cloud_
        vector[vector[float]] return_cloud
        vector[float] return_score
        ...
        void GetObjects(const float confidence_thresh, const float height_thresh, const int min_pts_num) except +MemoryError
...
cdef class PyCluster:
    ...
    def GetObjects2Py(self, float confidence_thresh, float height_thresh, int min_pts_num):
        self.c_cluster.GetObjects(confidence_thresh, height_thresh, min_pts_num)

    def Obstacle2Py(self):
        return self.c_cluster.obstacles_

    def Score2Py(self):
        return self.c_cluster.return_score

    def Typrname2Py(self):
        return self.c_cluster.type_name

    def Cloud2py(self):
        return self.c_cluster.return_cloud